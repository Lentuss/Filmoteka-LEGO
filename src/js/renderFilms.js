  ...
  return requestedFilms.results
    .map(
      ({
        ...
        <img class="main__movie-img" load="lazy" src="${posterPicture}" alt="${original_title}">
        <div class="main__movie-info">
        <h2 class="main__movie-title">${name}</h2>
        <p class="main__movie-genre">${genresForRender.join(
          ', '
        )}<span class="main__movie-year">${date}</span></p>
